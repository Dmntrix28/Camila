...
const MASTER_TL = timeline({
  ...
  onComplete: () => {
    ...
    // ⏳ Después de 5s ocultamos el mensaje final y mostramos el título inicial
    setTimeout(() => {
      title3.classList.add('hidden');
      title1.classList.remove('hidden'); // vuelve a aparecer el primer título
    }, 5000);
  },
  paused: true }).
...
MASTER_TL.addPause('LIGHTS_OUT', () => {
  ...
  // ⬇️ Espera 2 segundos y cambia el título
  setTimeout(() => {
...
BTN.addEventListener('click', () => {
  ...
  // 🔥 Ocultar título 1 y mostrar título 2
